...
public class Timer {
	...
	static private TimerThread thread () {
		synchronized (threadLock) {
			if (thread == null || thread.files != GdxGlue.files) {
	...
	static abstract public class Task implements Runnable {
		final ApplicationGlue app;
		long executeTimeMillis, intervalMillis;
		int repeatCount = CANCELLED;

		public Task () {
			app = GdxGlue.app; // Store which app to postRunnable (eg for multiple LwjglAWTCanvas).
	...
	static class TimerThread implements Runnable, LifecycleListener {
		...
		public TimerThread () {
			files = GdxGlue.files;
			GdxGlue.app.addLifecycleListener(this);
			resume();
		...
		public void run () {
			while (true) {
				synchronized (threadLock) {
					if (thread != this || files != GdxGlue.files) break;
		...
		public void dispose () { // OK to call multiple times.
			...
			GdxGlue.app.removeLifecycleListener(this);
		}
	}
}
